...
enum Mode { INDIVIDUAL, UNIFIED };
Mode currentMode = INDIVIDUAL;
unsigned long lastModeSwitch = 0;
const unsigned long modeInterval = 300000; // 30 seconds
...
Letter letters[] = {
  {0, NUM_LEDS_L, &primaryPalette, 0},
  {NUM_LEDS_L, NUM_LEDS_O1, &neonPalette, 32},
  {NUM_LEDS_L + NUM_LEDS_O1, NUM_LEDS_O2, &rainbowPalette, 64},
  {NUM_LEDS_L + NUM_LEDS_O1 + NUM_LEDS_O2, NUM_LEDS_V, &primaryPalette, 96},
  {NUM_LEDS_L + NUM_LEDS_O1 + NUM_LEDS_O2 + NUM_LEDS_V, NUM_LEDS_E, &neonPalette, 128}
};
...
void setup() {
  ...
  FastLED.addLeds<LED_TYPE, 9, COLOR_ORDER>(leds, NUM_LEDS_L + NUM_LEDS_O1 + NUM_LEDS_O2, NUM_LEDS_V).setCorrection(TypicalLEDStrip);
...
void loop() {
  ...
  if (currentMode == INDIVIDUAL) {
    ...
    for (int i = 0; i < NUM_LEDS; i++) {
      leds[i] = ColorFromPalette(rainbowPalette, t + (i * 255 / NUM_LEDS));
    }
  }

  FastLED.show();

  if (millis() - lastModeSwitch > modeInterval) {
    currentMode = (currentMode == INDIVIDUAL) ? UNIFIED : INDIVIDUAL;
    lastModeSwitch = millis();
  }
